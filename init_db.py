...
def create_database():
    with app.app_context():
        ...
        try:
            ...
            # Add is_active columns if they don't exist
...
def add_is_active_columns():
    ...
    print("🔄 Checking for is_active columns...")
    
    try:
        # Check if is_active exists in category table
        result = db.engine.execute("SELECT column_name FROM information_schema.columns WHERE table_name='category' AND column_name='is_active'")
        if result.rowcount == 0:
            db.engine.execute('ALTER TABLE category ADD COLUMN is_active BOOLEAN DEFAULT TRUE')
            print("  ✅ Added is_active to category table")
        else:
            print("  ℹ️  is_active already exists in category table")
    except Exception as e:
        print(f"  ℹ️  Category table: {str(e)}")
    
    try:
        # Check if is_active exists in product table
        result = db.engine.execute("SELECT column_name FROM information_schema.columns WHERE table_name='product' AND column_name='is_active'")
        if result.rowcount == 0:
            db.engine.execute('ALTER TABLE product ADD COLUMN is_active BOOLEAN DEFAULT TRUE')
            print("  ✅ Added is_active to product table")
        else:
            print("  ℹ️  is_active already exists in product table")
    except Exception as e:
        print(f"  ℹ️  Product table: {str(e)}")
